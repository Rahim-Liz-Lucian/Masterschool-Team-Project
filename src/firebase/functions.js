...
export function uploadProduct(user, product) {
  const productRef = doc(
    fireStore,
    `users/${user?.uid}/products/${product.uid}`
  );
  return setDoc(productRef, product);
}

export function findProduct(userPath, productId) {
  const productRef = doc(fireStore, `${userPath}/products/${productId}`);
  return getDoc(productRef, productId);
}
